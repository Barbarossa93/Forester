...
# cover | center | tile | full | extend | fill
wallpaperMode="fill"
...
darkmode_run_day="/home/barbarossa/.local/bin/theme_switcher.sh -l"
darkmode_run_night="/home/barbarossa/.local/bin/theme_switcher.sh -d"
...
get_sunpoll(){
    ...
    if [ "$currenttime" -ge "$sunriseMid" ] && [ "$currenttime" -lt "$twilightLate" ]; then
        sun_poll="DAY"
    else
        sun_poll="NIGHT"
    fi
...
setpaper_construct(){
    ...
    if [ "$oguri_enable" == "true" ];then 
        ...
        #hsetroot -"$wallpaperMode" "$wallpaperPath"/"$image".jpg
        /home/barbarossa/.local/bin/sun_animate.sh -a "$image"
    fi
