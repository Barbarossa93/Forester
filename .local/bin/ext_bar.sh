...
. "$HOME/Public/neat-things/colorschemes/forest-night-xresources/forest-night.sh"

mrect=( $(herbstclient monitor_rect "1" ) )
...
y=${mrect[1]}
rect=(
    $(bc -l <<< "$width * 0.8" | cut -d'.' -f1)
    36
    $(bc -l <<< "$width * 0.10 + $x" | cut -d'.' -f1)
    6
)

WIDTH=$(( ${rect[0]} -12 )) # bar width
HEIGHT=$(( ${rect[1]} - 12 )) # bar height
XOFF=$(( ${rect[2]} + 6 )) # x offset
YOFF=$(( ${rect[3]} + 6 )) # y offset
...
window_title() {
    ...
    #herbstclient --idle "window_title_changed" |
...
volume()
{
    while true; do
        ...
        if [[ ${mut} = "[off]" ]]; then
            ...
            label="${CYAN}${GLYVOL1}${CLR} %{A1:amixer set Master toggle:}%{A4:amixer set Master 5%+:}%{A5:amixer set Master 5%-:}$vol%%%{A}"
        elif [[ "$vol" -lt 40 ]]; then
            label="${CYAN}${GLYVOL2}${CLR} %{A1:amixer set Master toggle:}%{A4:amixer set Master 5%+:}%{A5:amixer set Master 5%-:}$vol%%%{A}"    
        elif [[ "$vol" -lt 60 ]]; then
            label="${CYAN}${GLYVOL3}${CLR} %{A1:amixer set Master toggle:}%{A4:amixer set Master 5%+:}%{A5:amixer set Master 5%-:}$vol%%%{A}"    
        elif [[ "$vol" -lt 80 ]]; then
            label="${CYAN}${GLYVOL4}${CLR} %{A1:amixer set Master toggle:}%{A4:amixer set Master 5%+:}%{A5:amixer set Master 5%-:}$vol%%%{A}"
        else
            label="${CYAN}${GLYVOL5}${CLR} %{A1:amixer set Master toggle:}%{A4:amixer set Master 5%+:}%{A5:amixer set Master 5%-:}$vol%%%{A}"
        fi
...
brightness() {
    while true; do
        ...
        if [[ "$rounded" -lt 25 ]]; then
            echo "BRIGHTNESS ${YELLOW}${GLYLIGHT1}${CLR} %{A4:xbacklight -inc 5:}%{A5:xbacklight -dec 5:}$rounded%%%{A}"
        elif [[ "$rounded" -lt 50 ]]; then 
            echo "BRIGHTNESS ${YELLOW}${GLYLIGHT2}${CLR} %{A4:xbacklight -inc 5:}%{A5:xbacklight -dec 5:}$rounded%%%{A}"    
        elif [[ "$rounded" -lt 75 ]]; then 
            echo "BRIGHTNESS ${YELLOW}${GLYLIGHT3}${CLR} %{A4:xbacklight -inc 5:}%{A5:xbacklight -dec 5:}$rounded%%%{A}"
        else
            echo "BRIGHTNESS ${YELLOW}${GLYLIGHT4}${CLR} %{A4:xbacklight -inc 5:}%{A5:xbacklight -dec 5:}$rounded%%%{A}"
        fi
...
echo "" | lemonbar -p -g ${rect[0]}x${rect[1]}+${rect[2]}+${rect[3]} -B "${BBG}" -d -n "bg" &
sleep 0.2
echo "" | lemonbar -p -g $(( ${rect[0]} - 10))x$(( ${rect[1]} - 10))+$(( ${rect[2]} + 5 ))+$(( ${rect[3]} + 5 )) -B "${BDR}" -d &
